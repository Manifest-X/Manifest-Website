...
Table styles are included in Manifest CSS or a standalone stylesheet, both referencing [theme](/docs/styles/theme) variables.
...
```html "Standalone"
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/mnfst@latest/lib/manifest.table.css" />
```
...
Default tables use the following [theme](/docs/styles/theme) variables:

| Variable | Purpose |
|----------|---------|
| `--color-surface-1`{copy} | Header and striped row background |
| `--color-line`{copy} | Row border color |
| `--spacing`{copy} | Cell padding base unit |
| `--radius`{copy} | Table border radius |
...
::: frame !bg-transparent text-base
<table>
    <thead>
        <tr>
            <th class="w-full">User</th>
