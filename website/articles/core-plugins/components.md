...
All components are rendered as a descendant of the `index.html` body, and cannot contain their own `<body>` tag.
...
::: frame "logo.html"
<svg class="h-7 fill-accent-content" viewBox="0 0 197 48" xmlns="http://www.w3.org/2000/svg">
        <g>
            <path clip-rule="evenodd" d="m0 24c15.2548 0 24-8.7452 24-24 0 15.2548 8.7452 24 24 24-15.2548 0-24 8.7452-24 24 0-15.2548-8.7452-24-24-24z" fill-rule="evenodd" />
...
::: brand icon="lucide:info"
Component paths and `<script>` tags inside component HTML execute as authored — that's the design. Keep the registered paths static (don't compose them at runtime from data sources or URL parameters) and treat any `<script>` inside a component as the same trust level as code in `index.html`.
:::
...
```html "special-page.html"
<body>
    <x-header id="specialHeader" class="text-slate-50 !bg-slate-500"></x-header>
    ...
</body>
```

::: frame "special-page.html"
<x-header-nested id="specialHeader" class="text-slate-50 !bg-slate-500"></x-header-nested>
:::
...
These custom attributes are created in the component source file using Alpine directives (dynamic attributes) like [x-text](https://alpinejs.dev/directives/text), [x-html](https://alpinejs.dev/directives/html), and [x-bind](https://alpinejs.dev/directives/bind), or Manifest's extended directives like [x-icon](/docs/elements/icons).

<div x-code-group>

```html "Empty Component"
...
If overrides with custom attributes are not required in every instance, place fallback logic in the component source to ensure optimal presentation.

<div x-code-group>

```html "Component"
<!-- Static classes apply if not overridden by the colors attribute -->
...
::: frame "Component"
<x-header-modified wordmark="Acme"></x-header-modified>
:::
...
```json "content.json"
{
    "specialHeader": {
        ...
        "colors": "!gap-0 [&_strong]:-ml-1 [&_strong]:mb-1 text-white bg-stone-600"
    },
    ...
}
```
...
A Manifest component can be embedded in any HTML like a WordPress site, Shopify storefront, static landing page, or an app built in another framework. Host the component files on a CDN or your own server, declare them in the project's `manifest.json`, then load Manifest's script and drop in the component tags.

<div x-code-group>

```html "any-host-page.html" copy
<!-- manifest.json with Manifest-framework fields added -->
<link rel="manifest" href="/manifest.json">
...
<!-- Drop components anywhere on the page -->
<x-pricing></x-pricing>
```
