...
Manifest is accessible by default. The framework's reset stylesheet, semantic element styles, and interactive plugins all ship with <a href="https://www.w3.org/WAI/WCAG21/quickref/" target="_blank">WCAG 2.1 Level AA</a> aligned behaviors — keyboard navigation, focus management, ARIA wiring, and live-region announcements are handled for you on every Manifest primitive. Authoring HTML against Manifest yields accessible output without extra configuration.
...
Manifest's advanced elements — including those supported by plugin scripts — implement the relevant <a href="https://www.w3.org/WAI/ARIA/apg/patterns/" target="_blank">WAI-ARIA Authoring Practices</a> patterns. There is no setup required beyond using the elements:
...
| [Sidebars](/docs/elements/sidebars) | Native popover dismissal with Escape; pair with `<dialog popover>` for a modal focus-trap variant |
...
Manifest's CSS animations and the View Transitions API respect `prefers-reduced-motion` automatically. Users with reduced-motion enabled see instant navigation instead of crossfade, and component animations resolve immediately rather than easing. No author handling required.
...
Authors retain responsibility for content-level criteria: heading hierarchy, image alt text, link discernibility, custom theme contrast ratios, and form label association.

---

## Automated Testing

Run the project linter to catch accessibility violations alongside other regressions:

```bash copy
npx mnfst-test
```

The runtime pass injects <a href="https://github.com/dequelabs/axe-core" target="_blank">axe-core</a> against a headless render of your project and reports failures with severity, the offending element, and — for color-contrast — the theme CSS variable driving the failing color. See [Testing](/docs/getting-started/testing) for full details.
